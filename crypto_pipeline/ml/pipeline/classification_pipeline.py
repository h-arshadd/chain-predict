...
threaded through to build_metadata() so persistence/model_loader.py can
...
def run_classification_pipeline(
    ...
    try:
        ...
        # classes comes from the trained model itself (model.classes_, per
        # heading 6/7's required classifier interface -- traditional and
        # deep learning classifiers both expose it), not re-derived from
        # y_test -- the label SET a classifier learned during train() is
        # what matters for reproducibility, not just whatever happens to
        # appear in this particular test split. model_kind is whichever
        # registry matched above -- this is what lets model_loader.load_run()

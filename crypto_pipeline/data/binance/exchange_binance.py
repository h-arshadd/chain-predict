...
class BinanceExchange:
    """
    Handles all Binance API interactions for fetching OHLCV candle data.
    Uses the python-binance library with futures klines (linear perpetual contracts).
    """
    ...
    def fetch_batch(self, symbol: str, interval: str, start_ms: int, end_ms: int) -> list:
        """
    ...
    def fetch_candles(self, symbol: str, timeframe: str, start_date, end_date: datetime, config: dict) -> pd.DataFrame:
        ...
        while current_start_ms < end_ms:
            ...
            while attempt < retries:
                try:
                    batch = self.fetch_batch(full_symbol, interval, current_start_ms, end_ms)
...
_binance_exchange = BinanceExchange()

def fetch_candles(symbol: str, timeframe: str, start_date, end_date: datetime, config: dict) -> pd.DataFrame:
    return _binance_exchange.fetch_candles(symbol, timeframe, start_date, end_date, config)

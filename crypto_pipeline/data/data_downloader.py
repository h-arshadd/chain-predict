...
def generate_expected_index(start_date, end_date: datetime, timeframe: str) -> pd.DatetimeIndex:
    """
...
def fill_missing_candles(df: pd.DataFrame, expected_index: pd.DatetimeIndex, method: str) -> pd.DataFrame:
    """
...
def fill_zero_volume(df: pd.DataFrame, method: str) -> pd.DataFrame:
    """
...
def download_data(config: dict, exchange_fetcher, conn):
    ...
    create_tables(conn, exchange, symbols=symbols, timeframes=timeframes)
    ...
    for symbol in symbols:
        for timeframe in timeframes:
            ...
            try:
                ...
                df = exchange_fetcher.fetch_candles(
                    symbol=symbol,
                    timeframe=timeframe,
                ...
                expected_index = generate_expected_index(actual_start, end_date, timeframe)

                # ── Step 6: Fill missing candles ───────────────────────────────
                df = fill_missing_candles(df, expected_index, filling_method)

                # ── Step 7: Fix zero volumes ───────────────────────────────────
                df = fill_zero_volume(df, zero_volume_method)

                # ── Step 8: Reset index ────────────────────────────────────────
                df = df.reset_index().rename(columns={"index": "date_time"})

                # ── Step 9: Insert into PostgreSQL ─────────────────────────────
                insert_candles(conn, exchange, symbol, timeframe, df)
